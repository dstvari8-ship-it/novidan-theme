<?php
	$networks = noozbeat_get_social_networks();
	$user     = array();
	$global   = array();
	$used     = array();
	...
	if ( ( in_the_loop() && count( $used ) > 0 ) || ( ! in_the_loop() && ( count( $used ) > 0 || $has_rss ) ) ) {
		...
		<ul class="social-icons">
			<?php
				foreach ( $networks as $network ) {
					if ( ! empty( $used[ $network['name'] ] ) ) {
						echo sprintf( '<li><a target="_blank" href="%s" class="icon-social"><i class="fa %s"></i></a></li>',
			...
			<?php if( ! in_the_loop() && $has_rss ): ?>
				<li><a target="_blank" href="<?php echo esc_url( get_theme_mod( 'rss_feed', get_bloginfo( 'rss2_url' ) ) ); ?>" class="icon-social"><i class="fa fa-rss"></i></a></li>
			<?php endif; ?>
		</ul>
		<?php
	}
